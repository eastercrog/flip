...
export default function BetSelectModal({
    ...
    return ( <
        ...
        p >
        SLIDER({
            parseFloat(lowerLimit / 10 ** selectedDecimals).toFixed(2)
        } - {
            " "
        } {
            parseFloat(upperLimit / 10 ** selectedDecimals).toFixed(2)
        } {
            selectedToken
        }) <
        /p> <
        ...
        <
        div className = "activeSliderContainer" > { /* sliderDiv is the creation of the slider with html and css. The actual slider is invisible and behind this makeshift slider */ } <
        ...
        /div> <
        /div> { /* the slider input component */ } <
        input onChange = {
            (_) => {
        ...
        input onChange = {
            (_) => {
                _.target.value * 10 ** selectedDecimals > upperLimit ?
                    setActiveBetAmount(upperLimit) :
                    setActiveBetAmount(_.target.value * 10 ** selectedDecimals);
            }
        }
        ...
        Current Bet: {
            ...
            activeBetAmount
                ?
                parseFloat(activeBetAmount / 10 ** selectedDecimals).toFixed(3) :
                0.0
        } {
            " "
        } {
