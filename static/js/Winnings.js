...
export default function Winnings({
    ...
    const payoutCompleteListener = useCallback(
        (better, token, winnings) => {
            if (account.address === better) {
                refreshWinnings();
                refreshTokenBalance();
            }
    ...
    const refreshWinnings = async () => {
        if (game.signer && account) {
            try {
                const bscf = await game._winnings(
                    account.address,
                    addresses[chainId].bscF
                );
                const bnb = await game._winnings(
                    account.address,
                    addresses[chainId].bnb
                );
    ...
    return ( <
        div className = "mb-8" >
        <
        div className = "text-white text-sm text-center" > WINNINGS < /div> <
        div className = "border-[#3A3A3A] px-4 py-2 border-[1px] rounded-md mt-2 text-white" > {
            selectedToken === "BNB" && ( <
                span onClick = {
            ...
            selectedToken === "BSCF" && ( <
                span onClick = {
                    () => claimWinnings(addresses[chainId].bscf)
                } >
                BSCF: {
                    parseFloat(unclaimedBSCF).toFixed(2)
                } <
                /span>
            )
        } <
